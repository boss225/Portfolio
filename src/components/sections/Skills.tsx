...
export default function Skills() {
  return (
    <section
      id="skills"
      className="py-12 sm:py-16 lg:py-18 px-4 sm:px-6 lg:px-30 mx-auto relative"
    >
      ...
      <div className="flex flex-col mb-8 sm:mb-12 lg:mb-16 relative">
        ...
        />
        <div
          className="absolute top-0 right-0 opacity-10 pointer-events-none select-none hidden sm:block"
          aria-hidden="true"
        >
          <span className="material-symbols-outlined text-[80px] sm:text-[100px] lg:text-[120px]">
            architecture
          </span>
        </div>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
        <div className="lg:col-span-2 glass-panel p-4 sm:p-6 lg:p-8 rounded-xl overflow-x-auto relative">
          <div>
            <table
              ...
              role="table"
              aria-label="Technical skills table"
            >
              <thead>
                <tr
                ...
                >
                  ...
                  <th className="pb-4 font-bold hidden sm:table-cell">
                    Years of Experience
                  </th>
                  <th className="pb-4 font-bold sm:hidden">Experience</th>
                  <th className="pb-4 font-bold hidden sm:table-cell">
                    Last Used
                  </th>
                </tr>
              ...
              <tbody className="text-xs sm:text-sm">
                {skills.map((skill) => (
                  ...
                  >
                    <td className="py-2 sm:py-3 md:py-4 font-medium pr-2">
                      {skill.name}
                    </td>
                    <td
                      className="py-2 sm:py-3 md:py-4 px-1 sm:px-2 text-center sm:text-left"
                      style={{ color: "var(--color-on-surface-variant)" }}
                    >
                      {skill.years}
                    </td>
                    <td
                      className="py-2 sm:py-3 md:py-4 font-bold text-xs sm:text-sm"
                      style={{
                        color: skill.isCurrent
                          ? "var(--color-secondary)"
          ...
          <div className="mt-8 sm:mt-10 lg:mt-12">
            ...
            <div className="flex flex-wrap gap-2 sm:gap-3">
              {tools.map((tool) => (
                ...
                >
                  <span
                    className={`material-symbols-outlined text-sm ${tool.color}`}
                  >
                    {tool.icon}
                  </span>
                  {tool.name}
        ...
        <div className="flex flex-col gap-6 sm:gap-8">
          {/* Education */}
          <div
            className="glass-panel p-6 sm:p-8 rounded-xl border-l-4"
